...
Q d(Q q){return q&0x0000ffffffffffffULL;}                                     // hi bits of pointer hold a header. 4 bit type 2 bit element size 2 bit shape. FUTURE: 256 byte min allocation and 256 byte alignment (8 low bits free, eliminate need for shift to get header)
...
Q zn(B t,B z,D m,D n){                                                        // contiguous allocation of data with same type and width, m metdata elements. n total data elements
  Z*o=malloc(sizeof(Z)+(2*4*m)+((1<<z)*n));                                   // there are two metadata vectors of length m. all metadata is dword
  Z h={t,m,n};*o=h;
  return wh(0,z,3,(Q)o);
}
...
void pr(Q q){
  if(0==q){return;}
  if(0==t(q)){
    if(3==sh(q)){for(D i=0;i<nz(q);i++){printf("%lld ",Dzi(q,i));}}
...
Q noZ(Q w){Q z=zn(3,ls(w),mz(w),nz(w));for(D i=0;i<2*mz(w);i++){Nz(z)[i]=Nz(w)[i];}for(D i=0;i<nz(w);i++){Dzid(z,i,!Dzi(w,i));};return z;}
...
Q plZZ(Q a,Q w){ // not atomic in the usual sense of a k implementation. 
  ...
  for(D i=0;i<nz(w);i++){Dzid(z,i,Dzi(a,i)+Dzi(w,i));}
  return z;
}
...
Q caVZ(Q a,Q w){Q z=zn(t(w),ls(w),1+mz(w),n(a)+nz(w));
  Oz(z)[0]=1;Oz(z)[1]=((D*)Dz(z))-Oz(z);
  *Nz(z)=1+*Nz(w);Nz(z)[1]=n(a);
  ...
  for(D i=0;i<nz(w);i++){Dzid(z,i+n(a),Dzi(w,i));}
  return z;
}
Q caZA(Q a,Q w){Q z=zn(t(w),ls(w),1+mz(a),1+nz(a));
  Oz(z)[0]=1;*Nz(z)=1+*Nz(a);
  for(D i=1;i<2*mz(a);i++){Nz(z)[i]=Nz(a)[i];}
  for(D i=0;i<nz(a);i++){Dzid(z,i,Dzi(a,i));}
  Nz(z)[mz(a)]=1;Oz(z)[mz(a)]=((D*)(Dz(z)+nz(a)))-Oz(z);Dzid(z,nz(a),d(w));
  return z;
}
Q caZV(Q a,Q w){Q z=zn(t(w),ls(w),1+mz(a),n(w)+nz(a));
  Oz(z)[0]=1;*Nz(z)=1+*Nz(a);
  for(D i=1;i<2*mz(a);i++){Nz(z)[i]=Nz(a)[i];}
  for(D i=0;i<nz(a);i++){Dzid(z,i,Dzi(a,i));}
  Nz(z)[mz(a)]=1;Oz(z)[mz(a)]=((D*)(Dz(z)+nz(a)))-Oz(z);
  for(D i=0;i<n(w);i++){Dzid(z,nz(a)+i,pi(w,i));}

  return z;
}
Q caZZ(Q a,Q w){Q z=zn(t(w),ls(w),mz(a)+mz(w),nz(a)+nz(w));
  D i=1,j=0;
  Oz(z)[0]=1;*Nz(z)=(*Nz(a))+(*Nz(w));
  printf("nz(z) %d *Nz(z) %d\n",nz(z),*Nz(z));
  for(j=1;j<*Nz(a);j++,i++){Oz(z)[i]=Oz(a)[j];Nz(z)[i]=Oz(a)[j];} // wrong
  ...
  for(i=0,j=0;i<nz(a);i++,j++){Dzid(z,i,Dzi(a,j));}
  for(j=0;j<nz(w);i++,j++){Dzid(z,i,Dzi(w,j));}
  return z;
}
