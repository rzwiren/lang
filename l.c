...
Q dvb(Q a,Q w,DV dv){ B ta=t(a),tw=t(w),sa=sh(a),sw=sh(w);D cta=d(ct(a)),ctw=d(ct(w));// dyadic verb broadcast
  ...
  return ac(0); // if we get here then a and w are not type 0 so work can be done on them
}
Q mvb(Q w,MV mv){ // monadic verb broadcast. will return data if it broadcasts. if not it returns a sentinel to communicate upwards either a length error or for the verb to do the work. 
  Q z;
  if(0==t(w)){
    z=1==sh(w)?tn(0,3,n(w)):zn(0,3,mz(w),n(w));
    for(D i=0;i<n(w);i++){
      Q wi=Giv(w,i);
      Q zi=mv(wi);  // mvb(wi,mv); ???
      if(5==t(zi)){ // control sentinel. if we get a control here that means the verb call just above couldn't do the work. must be error. return early (is that a good assumption?)
        return zi; // later: deallocate z before the return here. 
      }
      if(1==sh(w)){pid(z,i,zi);} else {Dzid(z,i,zi);}
    }
    return z;
  }
...
Q math(Q a,Q w,DV op){ // anything that gets here has been broadcasted. we can use the universal getter on both a and w
  ...
  printf(" math sha %d shw %d shz %d nz %d lz %d\n",sh(a),sh(w),shz,nz,lz);
...
Q pl_aa(Q a,Q w){ printf("pl_aa %lld %lld\n",a,w);return a+w;}                               // expect broadcastable situations. e.g atom atom,atom vec, atom zlist,vec vec, zlist zlist
Q ml_aa(Q a,Q w){ return a*w;}
Q pl(Q a,Q w){Q z=dvb(a,w,pl);if(5!=t(z)){return z;}if(c(z)){return z;}; return math(a,w,pl_aa);}
